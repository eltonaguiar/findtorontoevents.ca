#!/usr/bin/env python3
"""
Offline Performance Analyzer (Production) — Ranks all algorithms from DB trade data.
No live API calls needed. Outputs ranked CSV + JSON for dashboard consumption.

NOTE: This is the PRODUCTION version with full DB connectivity. If you see a
simplified pandas-only version at perf_analyzer_offline.py, it was replaced by
another AI and does NOT connect to the actual database.

Usage:
  python perf_analyzer_prod.py              # Analyze from DB
  python perf_analyzer_prod.py --csv FILE   # Analyze from CSV export
"""
import os
import sys
import json
...
def analyze(trades):
    ...
    for t in trades:
        ...
        if key not in algo_stats:
            algo_stats[key] = {
                'algorithm': algo, 'asset_class': asset,
                'total_trades': 0, 'wins': 0, 'losses': 0,
                'total_pnl': 0.0, 'total_pct': 0.0,
        ...
        s['total_pnl'] += pnl
        s['total_pct'] += pct
        s['total_capital_deployed'] += float(t.get('position_value_usd', 0))
    ...
    for key, s in algo_stats.items():
        ...
        results.append({
            'rank': 0, 'algorithm': s['algorithm'], 'asset_class': s['asset_class'],
            'total_trades': n, 'wins': s['wins'], 'losses': s['losses'],
            'win_rate_pct': round(win_rate, 2), 'avg_return_pct': round(avg_return, 2),
            'avg_pnl_usd': round(s['total_pnl'] / n, 2),
...
def save_results(results):
    ...
    with open(json_path, 'w') as f:
        json.dump({'generated': ts, 'total_algorithms': len(results), 'rankings': results}, f, indent=2)
...
def main():
    parser = argparse.ArgumentParser(description='Offline Performance Analyzer (Production)')
    parser.add_argument('--csv', help='Path to CSV trade export (skip DB)')
    args = parser.parse_args()
    print("=== Offline Performance Analyzer (Production) ===")
    if args.csv:
